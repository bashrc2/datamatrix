...
char
ecc200encode (unsigned char *t, int tl, unsigned char *s, int sl, char *encoding, unsigned int *lenp)
{
      ...
      case 'b':                // Binary
         {
            ...
            while (TP < tp)
            {                   // coding - see annex H - YES this includes the length bytes, believe it or not.
               t[TP] += (((TP + 1) * 149) % 255) + 1;
               TP++;
            }
            if (tp + 1 >= tl)
...
unsigned char *
iec16022ecc200_opts (iec16022ecc200_t o)
{
   ...
   {                            // find size
      ...
      {                         // find one that fits chosen encoding
         for (matrix = ecc200matrix; matrix->W; matrix++)
            if (ecc200encode (binary, matrix->bytes, o.barcode, o.barcodelen, encoding, 0)
                && (matrix->W == matrix->H || (!o.square && (!o.Wptr || o.Wptr != o.Hptr))))
               break;
      } else
      {
         int len;
